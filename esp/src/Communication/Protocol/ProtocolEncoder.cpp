...
void ProtocolEncoder::appendTouchCountAndTouches(uint8_t touch_count, const TouchFormat touch_format, const TouchVector &touches, float motor_angle) {
...
void ProtocolEncoder::appendRequestSenderConfig(const SenderConfig &config) {
    ...
    if (config.touch_count_and_touches) {
        data_.push_back(FORMAT_TOUCH_COUNT_AND_TOUCHES); count++;
        data_.push_back(config.touch_format); count++;
    }
